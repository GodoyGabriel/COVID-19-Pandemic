...
export default class CovidApi{
  static async getCases(){
    ...
    await http.get(`${uriCovidApi}/v1/cases`)
    .then(res => {
  ...
  static async getHistory({country, status}){
    let response = {status: true};
    await http.get(`${uriCovidApi}/v1/history?country=${country}&status=${status}`)
    .then(res => {
  ...
  static async getVaccinesInfo(country){
    let response = {status: true};
    await http.get(`${uriCovidApi}/v1/vaccines?country=${country}`)
    .then(res => {
      response.data = res.data.All;
    })
    .catch(err => {
